...
def main():
    ...
    print(f"T={T}, T/T2*={T/T2:.2f}, x*={x_star:.3f}, REAL grad={g_real:+.4f}, "
          f"1/λ={factor:.2f}.")
    print(f"  PSR raw RMSE {psr_raw_rmse:.4f} (wrong-sign {psr_raw_wrong:.0%}), "
          f"PSR rescaled RMSE {psr_resc_rmse:.4f}")
    ...
    axB.set_title(f"(B) error vs ε, N={N_SHOTS} shots — both arms fail on sign")
    ...
    fig.suptitle(f"Noisier regime T/T2*={T/T2:.2f}: FD's sign is wrong at BOTH "
                 f"small ε (shot noise) AND large ε (bias) —\nno ε is reliable; "
                 f"PSR (raw & rescaled) sit below the whole U.", fontsize=9.2)
    fig.tight_layout(rect=(0, 0, 1, 0.92))
    out = os.path.abspath(os.path.join(os.path.dirname(__file__), "..",
                                       "figures", "landscape_and_distance_noisy.png"))
