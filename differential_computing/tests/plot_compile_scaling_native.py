...
BLUE = "#2a78d6"      # series 1 — specialized
...
def main():
    ...
    ax.set_title("Base compilation: TFIM chain → pulse schedule",
    ...
    with open(os.path.join(FIGDIR, "compile_scaling_native_caption.txt"), "w") as fh:
        ...
        fh.write(
            "Compile-time scaling for the 1D TFIM chain on the rydberg2d AAIS "
            ...
            "layer (frozen chain geometry, bond-pruned ZZ/dressing lines, "
            "analytic warm start, sparse Jacobian) compiles the SAME AAIS with "
            f"slope ~{ss:.1f}, reaching n=1000 in {ts[-1]:.0f} s "
            f"(up to {speedup:.0f}x at overlapping sizes); the warm start is a "
            ...
            "the truncated dressing tail is declared in the plan "
            "(dropped_zz field, ~1.5% relative). Data: "
            "compile_scaling_native.json (median of repeated timings where "
            "compile < 5 s).\n")
    print("wrote", os.path.join(FIGDIR, "compile_scaling_native.png"))
    print(f"slopes: generic {sg:.2f}, specialized {ss:.2f}, branch {sb:.2f}")
